...
import { useSentOffers, useWithdrawOffer, type Offer } from '@/features/offers/use-offers'
...
export default function RequestsScreen() {
  ...
  const pendingIn = (incoming.data ?? []).filter((row) => row.status === 'pending')
  const handledIn = (incoming.data ?? []).filter((row) => row.status !== 'pending')
    ...
    <Screen>
      ...
      {nothingAtAll ? (
        <Callout title="Nothing waiting">
          When a colleague asks about your couch, or answers a trip you posted, it turns up
          here.
        </Callout>
      ) : null}
      ...
      {offers.length > 0 ? (
        <View style={styles.section}>
          <Text style={[typography.heading, { color: theme.text }]}>Your offers</Text>
          {offers.map((offer) => (
      ...
      {sentRequests.length > 0 ? (
        <View style={styles.section}>
          <Text style={[typography.heading, { color: theme.text }]}>You asked</Text>
...
function IncomingCard({ request }: { request: StayRequest }) {
  ...
  const nights = trip ? nightCount({ start: trip.start_date, end: trip.end_date }) : 0
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.bgSubtle, borderColor: theme.border }]}>
      ...
      <Text style={[typography.caption, { color: theme.textMuted }]}>
        {trip?.cities?.name} · {formatDateRange(trip?.start_date, trip?.end_date)} · {nights}{' '}
        night{nights === 1 ? '' : 's'}
      </Text>
        ...
        isCo ? (
          <>
            <Callout tone="neutral">
              Neither of you is hosting. Accepting shares your contact details so you can
              book something together.
            </Callout>
            <Button
              label="Accept and share contacts"
              onPress={() => {
                acceptCo.mutate(request.id, {
                  onSuccess: (result) => router.push(`/contact/${result.grantedWith}`),
                })
              }}
...
function OutgoingCard({ request }: { request: StayRequest }) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.bgSubtle, borderColor: theme.border }]}>
      ...
      <Text style={[typography.caption, { color: theme.textMuted }]}>
        {trip?.cities?.name} · {formatDateRange(trip?.start_date, trip?.end_date)}
      </Text>

      {request.status === 'pending' ? (
        <TextLink label="Withdraw" onPress={() => withdraw.mutate(request.id)} />
      ) : null}
    </View>
  )
}
...
function SentOfferCard({ offer }: { offer: Offer }) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.bgSubtle, borderColor: theme.border }]}>
      ...
      <Text style={[typography.caption, { color: theme.textMuted }]}>
        {nights} night{nights === 1 ? '' : 's'} · {offer.cities?.name ?? offer.trips?.cities?.name}{' '}
